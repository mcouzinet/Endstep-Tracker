...
- `content.js` persiste les fiches ; `dashboard.html` les affiche ; `meta.js` reconnaît le deck adverse.
...
```bash
node test/replay.test.js && node test/meta.test.js
```

Le premier rejoue une vraie séquence de messages capturée (Bo3 contre l'IA) et vérifie la fiche produite ; le second vérifie la reconnaissance du deck adverse sur un métagame réduit.
