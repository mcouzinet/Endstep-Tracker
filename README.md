...
**Meilleur coup.** Le laboratoire `Endstep-coach` peut rejouer chaque décision enregistrée dans Forge et classer les options comme le ferait son bot (main adverse tirée au sort depuis l'archétype reconnu) : Données → *Exporter une sauvegarde (JSON)*, puis `node ~/Developer/Endstep-coach/bot/coach-replay.js <export.json> analysis.json`, puis Données → *Importer une analyse…*. Le bloc Coach gagne deux colonnes : le meilleur coup selon le modèle et l'écart avec le tien (en points de probabilité de victoire). L'analyse est gardée à part (`ana:<matchId>`), exportée avec la sauvegarde, supprimée avec le match. Non rejoué : les décisions en réponse (pile non vide), les blocs, les mulligans, les choix internes d'un sort.
