...
(() => {
  ...
  const store = new Map(); // matchId -> { rec, rt }
  ...
  async function onMessage(kind, data) {
    if (kind === 'ws') {
      ...
      if (entry) { T.applyMeta(entry.rec, ctx.meta[data.id], ctx); save(entry.rec); }
    }
  }
  ...
  async function onFrame(f) {
    ...
    save(rec, gameEnded || rec.status !== 'active');
    setLive(rec.status === 'active');
  }

  function save(rec, immediately) {
    clearTimeout(timers.get(rec.id));
    const write = () => { timers.delete(rec.id); chrome.storage.local.set({ ['match:' + rec.id]: rec }); };
    if (immediately) write();
    else timers.set(rec.id, setTimeout(write, 1000));
  }
  ...
  window.addEventListener('pagehide', () => {
    for (const id of [...timers.keys()]) { clearTimeout(timers.get(id)); chrome.storage.local.set({ ['match:' + id]: store.get(id).rec }); }
  });
